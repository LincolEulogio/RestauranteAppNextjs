...
interface CartPaymentMethodSelectorProps {
    paymentMethod: "card" | "yape" | "plin" | "cash" | null
    setPaymentMethod: (method: "card" | "yape" | "plin" | "cash" | null) => void
}
    ...
    return (
        <>
            ...
            <div className="space-y-3">
                ...
                {/* Efectivo */}
                <div
                    className={`flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition-all duration-200 ${paymentMethod === "cash"
                        ? "border-green-500 bg-green-500/5 shadow-sm"
                        : "border-border hover:border-green-400/50 hover:bg-accent"
                        }`}
                    onClick={() => setPaymentMethod("cash")}
                >
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${paymentMethod === "cash" ? "border-green-500" : "border-muted-foreground/30"
                        }`}>
                        {paymentMethod === "cash" && (
